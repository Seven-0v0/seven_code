...
编译产物：
- `build/apps/blinky_f103/blinky_f103.elf` — 带调试信息
- `build/apps/blinky_f103/blinky_f103.bin` — 纯二进制（烧录用）
- `build/apps/blinky_f103/blinky_f103.hex` — Intel HEX
...
```
[BOOT] STM32F103C8T6 FreeRTOS Multi-Task System
[OK][led_task] toggle state=A
[OK][iwdg_task] refresh
[OK][led_task] toggle state=B
[OK][log_task] alive uptime=2045 freertos_tick=2045
[OK][led_task] toggle state=A
```
...
### Step 1：理解任务
- 读取 `apps/blinky_f103/src/main.c` 理解当前固件逻辑
- 确定需要修改的代码位置

### Step 2：修改代码
- 编辑 `apps/blinky_f103/src/main.c`（或创建/修改其他 `.c/.h` 文件）
- 如新增 `.h` 文件，放到 `apps/blinky_f103/include/`
- 如新增 `.c` 文件，放到 `apps/blinky_f103/src/` 并更新 `apps/blinky_f103/CMakeLists.txt` 中的 `SOURCES`
...
│   └── bluepill_f103c8/
│       ├── board.h             # 引脚定义
│       └── FreeRTOSConfig.h    # RTOS 配置（不要改）
├── bsp/
│   └── stm32/f1/
│       ├── startup/            # 启动文件（不要改）
│       └── STM32F103XB_FLASH.ld  # 链接脚本（不要改）
├── cmake/
│   └── chips/
│       └── stm32f103c8.cmake   # 芯片配置（不要改）
├── middleware/
│   └── FreeRTOS-Kernel/        # FreeRTOS 内核（submodule）
├── tools/
